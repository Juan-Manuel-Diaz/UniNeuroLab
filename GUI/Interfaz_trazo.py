...
from PyQt5.QtWidgets import QApplication, QMainWindow, QVBoxLayout, QWidget # Aca agrego estos 2
...
class MainApp(QMainWindow):
    ...
    def render(self):
        ...
        label1= QLabel("BIENVENIDO AL TEST_01 DIGITAL \n Aqui cargaremos los datos\n de los individuos \n Es el INPUT de la aplicacion")
        label1.setStyleSheet("background: green; color: yellow")
        label1.setAlignment(Qt.AlignCenter) #centrar el label dentro del QLabel
        ...
        label2.setAlignment(Qt.AlignCenter) #centrar el label dentro del QLabel
        ...
        btn1.clicked.connect(self.slot1)  ## --> con connect tenemos que programar un slot (handler si o si) que pasa luego del click
    ...
    def show_text_NA(self):
        print("El nombre y apellido del usuario es: ", self.user_input1.text(), self.user_input2.text())
...
class MyWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.bolitas=[b1, b2, b3, b4, b5, b6, b7, b8, b9, b10, b11, b12, b13, b14, b15, b16, b17, b18, b19, b20, b21, b22, b23, b24, b25]
    ...
    def initUI(self):
        ...
        for i in range(len(self.bolitas)):
            ...
            self.bolitas[i].t_Obj.setGeometry(self.bolitas[i].g1, self.bolitas[i].g2, self.bolitas[i].g3, self.bolitas[i].g4)
            self.bolitas[i].t_Obj.setStyleSheet(self.bolitas[i].setStyleSheet)
            self.bolitas[i].t_Obj.clicked.connect(lambda ch, val=i: self.cambioColor(val))             
    ...
    def cambioColor(self, i):
        ...
        if numero_estado1+1 == int(self.bolitas[i].valor):
            self.bolitas[i].t_Obj.setStyleSheet("background-color: green; color: white; border-radius : 15; border : 1px solid green")
            for x,e in enumerate(tr):
                if e == 1:
                    self.bolitas[x].t_Obj.setStyleSheet(self.bolitas[x].setStyleSheet)
                    tr[x] = 0

            print("Color Verde")

            dist.append(math.sqrt(((self.bolitas[i].g1-x_old)**2)+((self.bolitas[i].g2-y_old)**2)))
            x_old = self.bolitas[i].g1
            y_old = self.bolitas[i].g2

            bol_time_ver.append(round((time.time()-inicio)-bol_time_ver[-1],3))
            ...
            print("Color Rojo")
            tr[i] = 1
            self.bolitas[i].t_Obj.setStyleSheet("background-color: red; color: white; border-radius : 15; border : 1px solid red")
        ...
        if numero_estado1+1 == int(self.bolitas[i].valor): numero_estado1 += 1
